...
// Pitcher throwing variables
let isThrowing = false;
let throwWindUp = 0;
let throwRelease = 0;
let pitcherArmAngle = 0;
...
function createBall() {
  ...
  // Calculate hand position based on character
  const handOffsetX = 20; // Adjust based on your character image
  const handOffsetY = -3; // Adjust based on your character image
  
  ball = {
    x: characterX + handOffsetX,
    y: characterY - 3 + handOffsetY,
    radius: BALL_RADIUS,
    vx: 0,
    vy: 0,
    active: true,
...
// SINGLE VERSION OF drawPlayer() - USING IMAGE
function drawPlayer() {
  ctx.save();
  ...
  if (characterImageLoaded) {
    // Calculate image dimensions (adjust as needed for your image)
    const imgWidth = 100;
    const imgHeight = 150;
    
    // Adjust position based on throwing animation
    let offsetX = 0;
    let offsetY = 0;
    
    if (isDragging || isThrowing) {
      // Add slight movement during throwing
      offsetX = Math.sin((throwWindUp + throwRelease) * Math.PI) * 3;
    }
    
    ctx.drawImage(
      characterImage,
      characterX - imgWidth/2 + offsetX, 
      characterY - CHAR_HEIGHT - 20 + offsetY,
      imgWidth,
      imgHeight
    );
  } else {
    // Fallback: draw a simple character if image fails to load
    console.log("Image not loaded, using fallback character");
    ctx.fillStyle = '#2E8B57';
    ctx.fillRect(characterX - CHAR_WIDTH/2, characterY - CHAR_HEIGHT, CHAR_WIDTH, CHAR_HEIGHT);
    
    ctx.fillStyle = '#228B22';
    ctx.beginPath();
    ctx.arc(characterX, characterY - CHAR_HEIGHT - 10, 15, 0, Math.PI * 2);
    ctx.fill();
  }
  
  // Position ball in hand during wind-up
  if (ball && ball.held) {
  // Base hand position for spike-char1.png
  const baseHandX = 20;
  const baseHandY = -3;
  
  // Purely visual animation - doesn't affect physics
  if (isDragging || isThrowing) {
    // Visual wind-up animation only
    const throwProgress = isThrowing ? throwRelease : throwWindUp;
    const visualOffsetX = Math.cos(throwProgress * Math.PI * 1.5) * 10; // Smaller visual effect
    const visualOffsetY = Math.sin(throwProgress * Math.PI) * 5; // Smaller visual effect
    
    ball.x = characterX + baseHandX + visualOffsetX;
    ball.y = characterY - 3 + baseHandY + visualOffsetY;
  } else {
    // Normal holding position
    ball.x = characterX + baseHandX;
    ball.y = characterY - 3 + baseHandY;
  }
}
...
function drawBall() {
  ...
  if (ball.held) {
    // Ball position is handled in drawPlayer for pitcher animation
  }
...
function drawAimingLine() {
  ...
  // Use the actual hand position for aiming line, not the animated ball position
  const baseHandX = 20;
  const baseHandY = -3;
  const startX = characterX + baseHandX;
  const startY = characterY - 3 + baseHandY;
  ...
  const dx = startX - dragEnd.x;
  const dy = startY - dragEnd.y;
...
function drawTrajectory() {
  ...
  // Use the actual hand position for trajectory calculation, not the animated ball position
  const baseHandX = 20;
  const baseHandY = -3;
  const startX = characterX + baseHandX;
  const startY = characterY - 3 + baseHandY;
  
  const dx = (startX - dragEnd.x) * 0.1 * power;
  const dy = (startY - dragEnd.y) * 0.1 * power;
...
function initGame() {
  // Load character image
  characterImage.src = 'image/spike-char1.png';
...
// Mouse event handlers with pitcher animation
...
function handleMouseMove(e) {
  ...
  // Use the actual hand position for power calculation, not the animated ball position
  const baseHandX = 20;
  const baseHandY = -10;
  const startX = characterX + baseHandX;
  const startY = characterY - 3 + baseHandY;
  
  // Calculate power based on drag distance from hand position
  const dx = startX - dragEnd.x;
  const dy = startY - dragEnd.y;
  const dist = Math.min(Math.sqrt(dx*dx + dy*dy), 150);
  power = dist / 150;
...
function handleMouseUp() {
  ...
  // Use the actual hand position for physics calculation
  const baseHandX = 20;
  const baseHandY = -3;
  const startX = characterX + baseHandX;
  const startY = characterY - 3 + baseHandY;
  
  // Set ball to the actual hand position (not animated position)
  ball.x = startX;
  ball.y = startY;
  
  // Ball physics - use the consistent starting position
  const dx = (startX - dragEnd.x) * 0.1 * power;
  const dy = (startY - dragEnd.y) * 0.1 * power;
...
// Create intro particles (if needed)
function createIntroParticles() {
  // Add particle creation logic if needed
}
